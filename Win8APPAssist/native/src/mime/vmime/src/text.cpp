	...
	const string::size_type end, string::size_type* newPosition)
{
	...
	// 判断是否需要合并，简单如果所有word的charset一样且皆非ASCII编码，则全部合并。。
	// by yangchenwei@corp.netease.com 2012-09-17
	bool bNeedEmerge = true;
	if (words.size() > 1)
	{
		std::string sCharset = words[0]->getCharset().getName();
		if (sCharset.compare(charsets::US_ASCII) != 0)
		{
			for (int i=1; i<words.size(); i++)
			{
				if (words[i]->getCharset().getName().compare(charsets::US_ASCII) == 0
					|| words[i]->getCharset().getName().compare(sCharset) != 0)
				{
					bNeedEmerge = false;
					break;
				}
	...
	if (bNeedEmerge)
	{
		...
		for (int i=0; i<words.size(); i++)
			buffer += words[i]->getBuffer();
		ref <word> w = vmime::create<word>(buffer, words[0]->getCharset());
		m_words.push_back(w);
	}
	else
	{
