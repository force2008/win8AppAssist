	...
	const string::size_type end, string::size_type* newPosition)
{
	...
	// 增加对逗号间隔的兼容
	//while (pos < end && (parserHelpers::isSpace(buffer[pos]) || buffer[pos] == ','))
	//	++pos;

	// modified by yangchenwei@corp.netease.com 2012-12-07
	// 忽略'<'字符前的所有字符
	...
	if (pos != end)
	{
		...
		// modified by yangchenwei@corp.netease.com 2012-08-11
		while (pos < end)
		{
			if (buffer[pos] == '>')
			{
			...
			if (!parserHelpers::isSpace(buffer[pos]) && buffer[pos] != ',')
				++pos;
		}
...
// GenerateID, TODO:		// modified by qfliu@corp.netease.com	2012.7.22
messageId messageId::generateId()
{
	...
	return (messageId(left.str(), "netease"));
}
