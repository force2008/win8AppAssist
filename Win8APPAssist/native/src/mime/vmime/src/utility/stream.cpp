...
inputStreamStringAdapter::inputStreamStringAdapter(const string& buffer,
	...
	: m_buffer(buffer, begin, end), m_begin(begin), m_end(end), m_pos(begin)
{
}
	...
	(value_type* const data, const size_type count)
{
	if (m_pos + count >= m_end)
	{
		...
		std::copy(m_buffer.begin() + m_pos, m_buffer.end(), stdext::checked_array_iterator<value_type*>(data, m_buffer.end() - m_buffer.begin() - m_pos));
		m_pos = m_end;
		return (remaining);
	}
	else
	{
		std::copy(m_buffer.begin() + m_pos, m_buffer.begin() + m_pos + count, stdext::checked_array_iterator<value_type*>(data, count));
		m_pos += count;
		return (count);
	}
}
	...
	(value_type* const data, const size_type count)
{
	...
	if (count > remaining)
	{
		std::copy(m_buffer.it_begin() + m_pos, m_buffer.it_end(), stdext::checked_array_iterator<value_type*>(data, m_buffer.it_end() - m_buffer.it_begin() - m_pos));
		m_pos = m_buffer.length();
		return (remaining);
	}
	else
	{
		std::copy(m_buffer.it_begin() + m_pos, m_buffer.it_begin() + m_pos + count, stdext::checked_array_iterator<value_type*>(data, count));
		m_pos += count;
		return (count);
	}
}
	...
	(value_type* const data, const size_type count)
{
	...
	if (remaining < count)
	{
		std::copy(m_buffer + m_pos, m_buffer + m_pos + remaining, stdext::checked_array_iterator<value_type*>(data, remaining));
		m_pos += remaining;
	...
	{
		std::copy(m_buffer + m_pos, m_buffer + m_pos + count, stdext::checked_array_iterator<value_type*>(data, count));
		m_pos += count;
