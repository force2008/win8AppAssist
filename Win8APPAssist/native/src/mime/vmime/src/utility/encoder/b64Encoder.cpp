...
std::string b64Encoder::encode(Windows::Storage::Streams::DataReader^ in, utility::progressListener* progress)
{
	std::string outbuf;
	...
	const int propMaxLineLength = 76; // 用固定值
	...
	while (in->UnconsumedBufferLength > 0)
	{
		// Get 3 bytes of data
		int count = 0;

		while (count < 3 && in->UnconsumedBufferLength > 0)
...
utility::stream::size_type b64Encoder::encode(const char* in, int nLength, utility::outputStream& out, utility::progressListener* progress)
{
	const int propMaxLineLength = getProperties().getProperty <int>("maxlinelength", -1);
	//const int propMaxLineLength = 76; // 用固定值
	...
	while (bufferPos < nLength)
	{
		// Get 3 bytes of data
		int count = 0;

		while (count < 3 && bufferPos < nLength)
			bytes[count++] = in[bufferPos++];
