	...
	if (!m_objects.empty())
	{
		     ...
		     it != m_objects.end() ; ++it)
		{
			...
			if ((*it)->getName().length() != 0)
				objPart->getHeader()->ContentDisposition().dynamicCast <contentDispositionField>()->setFilename(vmime::word((*it)->getName(), charset(charsets::UTF_8)));
...
const string htmlTextPart::addObject(ref <contentHandler> data, const word& name, const std::string& id,
	const vmime::encoding& enc, const mediaType& type)
{
	...
	m_objects.push_back(vmime::create <embeddedObject>(data, enc, cid, name, type));
...
{
	return addObject(data, word(""), std::string(""), encoding::decide(data), type);
}
...
const string htmlTextPart::addObject(const string& data, const mediaType& type)
{
	...
	ref <word> w = create<word>("");
	return addObject(cts, word(""), std::string(""), encoding::decide(cts), type);
}
...
htmlTextPart::embeddedObject::embeddedObject
	(ref <contentHandler> data, const encoding& enc,
	 const string& id, const word& name, const mediaType& type)
	: m_data(data->clone().dynamicCast <contentHandler>()),
	  m_encoding(enc), m_name(name), m_id(id), m_type(type)
{
}
...
string htmlTextPart::embeddedObject::getName() const
{
	return m_name.getConvertedText(charset(charsets::UTF_8));
}
