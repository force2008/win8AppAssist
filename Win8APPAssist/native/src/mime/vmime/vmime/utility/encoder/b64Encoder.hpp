...
public:
	...
	std::string encode(Windows::Storage::Streams::DataReader^ in, utility::progressListener* progress = NULL);
	utility::stream::size_type encode(const char* in, int nLength, utility::outputStream& out, utility::progressListener* progress = NULL);
