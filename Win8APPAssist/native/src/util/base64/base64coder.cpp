...
//编码时候的每行的长度
unsigned int CBase64Coder::m_LineWidth = 76;   //lxl modify from 80  2010-03-30
//------------------------------------------------------------------------------

//Base64编码表
const char CBase64Coder::BASE64_ENCODE_TABLE[64] = {
	...
	//Base64解码表
const unsigned int CBase64Coder::BASE64_DECODE_TABLE[256] = {
...
//Quoted编码表
const byte CBase64Coder::QUOTED_ENCODE_TABLE[256] = {
	255, 255, 255, 255, 255, 255, 255, 255, // 00 -  07
...
//获取Base64编码长度
biguint CBase64Coder::Base64EncodeSize(biguint iSize)
{
	biguint nSize, nCR;
	nSize = (iSize + 2) / 3 * 4 ;
...
//获取Base64解码长度
biguint CBase64Coder::Base64DecodeSize(biguint iSize)
{
	return (iSize + 3) / 4 * 3;
}
...
//获取UUCode编码长度
biguint CBase64Coder::UUEncodeSize(biguint iSize)
{
	biguint nSize, nCR;
	nSize = (iSize + 2) / 3 * 4 ;
...
//获取UUCode解码长度
biguint CBase64Coder::UUDecodeSize(biguint iSize)
{
	return (iSize + 3) / 4 * 3;
}
...
//获取Quoted编码长度
biguint CBase64Coder::QuotedEncodeSize(biguint iSize)
{
	biguint nSize = iSize * 3 + 1;
	biguint nCR = nSize / m_LineWidth;
	nSize+= nCR * 3;
	return nSize;
}
...
//Base64编码
int CBase64Coder::base64_encode(const char *pSrc, unsigned int nSize, char *pDest)
{
...
int CBase64Coder::base64_decode(const char *pSrc, unsigned int nSize, char *pDest)
{
...
//UUCode编码
int CBase64Coder::UU_encode(char *pSrc, unsigned int nSize, char *pDest)
{
	if ((pSrc == NULL) || (pDest == NULL) || (nSize <= 0)) return 0;
	...
	while (InPtr < InLimitPtr)
	{
		...
		while ((InPtr < InLimitPtr) && (Len < m_LineWidth))
		{
			...
			{
				if (OutPtr[i] == NULL)
					OutPtr[i] = '`';//(byte) (OutPtr[i] + 0x60);
				else 
					OutPtr[i] = ' ' + OutPtr[i];//(byte) (OutPtr[i] + 0x20);
			}	
			OutPtr+=4;
			Len+=4;
		}
...
//UUCode解码
int CBase64Coder::UU_decode(char *pSrc, unsigned int nSize, char *pDest)
{
	char C[4]; //临时阅读变量
	char Tmp;
	int CurrIndex, Index;
...
//Quoted编码
int CBase64Coder::quoted_encode(char *pSrc, unsigned int nSize, char *pDest)
{
	unsigned int Len;
	byte B; //临时阅读变量
	char *InPtr, *InLimitPtr;
...
//Quoted解码
int CBase64Coder::quoted_decode(char *pSrc, unsigned int nSize, char *pDest)
{
	if ((pSrc == NULL) || (pDest == NULL) || (nSize <= 0)) return 0;
	...
	while (pSrc < InLimitPtr)
	{
		...
		else if (C[0] == '_') // 空格 http://www.freesoft.org/CIE/RFC/1522/6.htm
		{
			*pDest++ = ' ';
		}
		else //否则直接输出
...
//十进制-->十六进制
char CBase64Coder::DecToHex( char B)
{
...
//十六进制-->十进制
char CBase64Coder::HexToDec( char C )
{
